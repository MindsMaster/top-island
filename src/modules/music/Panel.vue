...
function onScrubStart() {
  if (seekable.value) musicState.isScrubbing = true;
}
...
const positionLabel = computed(() => (hasTimeline.value ? formatTimeMs(snap.positionMs) : '--:--'));
const durationLabel = computed(() => (hasTimeline.value ? formatTimeMs(snap.durationMs) : '--:--'));
...
<template>
  ...
  <div class="large-progress" :class="{ disabled: !hasTimeline }">
    <span class="progress-time">{{ positionLabel }}</span>
    <input
      type="range"
      class="progress-slider-lg"
      :class="{ readonly: !seekable }"
      min="0"
      :max="snap.durationMs || 100"
      step="1000"
