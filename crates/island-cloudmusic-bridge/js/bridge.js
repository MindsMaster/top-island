// 注入网易云渲染进程 经本地 WebSocket 上报播放状态
//
// NCM 3.1 事件模型（3.1.39 实测）
//   audioplayer.load(playId, ...)            call 比 onLoad 早
...
//
// 曲目身份是 songId 即 playId 前缀 后缀是播放会话 seek 时会变 不算切歌
(function () {
  'use strict';
  ...
  function enterTrack(playId) {
    ...
    cur.playing = true; // NCM 只在要播时才 load 没播会被 onPlayState 纠正
  }
